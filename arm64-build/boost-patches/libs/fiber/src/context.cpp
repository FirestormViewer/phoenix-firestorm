...
struct context_initializer {
    ...
    void deinitialize()
    {
        ...
        // ARM64 Windows / winfib: deleting the scheduler here runs
        // scheduler::~scheduler -> context::active()->suspend() -> SwitchToFiber while
        // this runs from a thread_local destructor during thread exit (__dyn_tls_dtor).
        // By that point the OS has torn down the thread's fiber state, so SwitchToFiber
        // dereferences a null fiber and crashes (e.g. the texture-cache purge thread).
        // The thread is dying, so leak the per-thread fiber state instead of switching.
        // Bounded: only affects fiber-using threads that exit; reclaimed at process end.
        (void)active_;
#else
...
// ARM64 Windows PAC fix: replace the thread_local static in context::active()
// with a heap-allocated context_initializer stored via plain thread_local pointer.
// MSVC's thread_local static uses InitOnceExecuteOnce for per-access guard checks;
// during thread exit, the guard resets and re-entry triggers InitOnceExecuteOnce
// whose autibsp crashes with FAST_FAIL_POINTER_AUTH_INVALID_RETURN_ADDRESS on ARM64.
// Plain thread_local variables use __dyn_tls_init/__dyn_tls_dtor instead, with no
// per-access guard checks, avoiding the PAC crash entirely.
static thread_local context_initializer * s_active_initializer{ nullptr };
static thread_local bool s_active_dtor_running{ false };

struct active_initializer_cleanup {
    ~active_initializer_cleanup() {
        if ( s_active_initializer) {
            s_active_dtor_running = true;
            context_initializer * p = s_active_initializer;
            s_active_initializer = nullptr;
            delete p;
        }
    }
};
static thread_local active_initializer_cleanup s_active_cleanup{};
...
context::active() noexcept {
    if ( ! s_active_initializer && ! s_active_dtor_running) {
        s_active_initializer = new context_initializer{};
    }
