 ...
 * class1/effects/glowExtractF.glsl. The RTX block lifts the bloom
 * extraction threshold response for a richer cinematic glow.
 */
...
void main()
{
    ...
    // RTX bloom boost: raise the extracted glow intensity so the glare pass
    // reads stronger highlights (keeps the viewer's original threshold math).
    float rtx_boost = 1.0;
  #if defined(MKO_RTX_QUALITY_Cinematic)
    rtx_boost = 1.25;
  #elif defined(MKO_RTX_QUALITY_Balanced)
    rtx_boost = 1.10;
  #endif
    lum = clamp(lum * rtx_boost, 0.0, 1.0);
#endif
