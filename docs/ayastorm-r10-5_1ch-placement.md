...
- **#5 routing 診断 log 検証**: `Stream3DRoutingDiagnostic=true` で §4.4 通りの log 出力確認 (r10 時点では 6ch source の grep 確認のみ。2ch×6prim placement での fallback 文言検証 + chat 通知化 + Preferences UI は r10.x で対応 → `docs/ayastorm-r10.x-routing-diag-chat.md`)
...
- [~] **routing 診断 log**: r10 では 6ch source の log emit を grep のみで確認、2ch×6prim placement での fallback 文言出力は実機未検証だった。本来意図 (chat 通知 + Preferences UI) との乖離も判明。r10.x で chat 通知化 + 5 ケース実機検証を完了 (詳細: `docs/ayastorm-r10.x-routing-diag-chat.md`)
