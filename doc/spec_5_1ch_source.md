...
| 指標 | 目標 |
|---|---|
| 5.1 ソース連続再生 5 分 | dropout 0、CPU 使用率 r8 比 +3% 未満 (downmix 計算分) |
| URL 動的切替 (2ch ↔ 6ch) × 10 回 | crash / hang / dropout 0、再生再開 1 秒以内 |
| 非対応 ch 数の URL | エラー通知 + 再生停止、crash / hang なし |
| `Stream3DEnabled` トグル × 50 回 (6ch source) | r8 と同様に binding が正しく start/stop |
...
- `ch=FL|FR|C|LFE|SL|SR` 追加 (6 種を本格 placement)
- ring `n_tracks=6` への切替、reader 側 (`LLPositionalStreamMulti::pcmReadCallback`) で各 ch を直接 track index から取り出す
- 配置ガイドライン (BS.775 角度) を仕様に明文化、sweet spot 不在を明文化
- LFE 扱い: r8 §9.1.4 (2) で「(p) 普通のスピーカーと同等」を推奨。r9 の downmix で LFE を含めた経験を踏まえ確定
- 詳細は別仕様書 `doc/spec_5_1ch_placement.md` (r10 着手時に新規作成) に切り出す
...
### 9.4 AYA 用 5.1 配信ランチャ (任意、r10 以降)

GUI 配信アプリ (butt 等) で 5.1ch を Opus surround で Icecast に流せる選択肢が出てくるまでの繋ぎとして、AYA 個人用に ffmpeg 起動を最小 GUI でラップする選択肢を残す。

候補:

- **ffmpeg + zenity / yad** (Linux): 素材選択 → Icecast 接続情報入力 → 1 クリック開始の最小ダイアログ。1 日工数程度
- **ffmpeg + PowerShell GUI** (Windows): 同等のものを WinForms で
- **Carla rack に ffmpeg を巻き込む**: JACK 経由で 6ch を取り出して ffmpeg にパイプ、Carla 上の preset として保存

これらは AYA 個人運用の道具で AYAstorm リリース成果物には含めない。GitHub の AYA 個人レポジトリに置くか、AYAstorm リポジトリの `tools/streaming/` に置くかは r10 以降の判断。

butt が将来 Opus mapping_family 1 をサポートしたら本項目は不要になる (上流ツールに乗る方が長期的には保守ラク)。
