...
- `ch` enum に `FL|FR|C|LFE|SL|SR` を追加 (ch:M で混ぜずに各 ch を個別スピーカーへ振る用途)
- §6 候補の `Stream3DDownmixGain` / `Stream3DAcceptMultichannel` 追加判断 (r9 では不要だった)
- SIMD 化の判断 (P10 で +1.09% に収まったため r10 でも当面不要見込み)
