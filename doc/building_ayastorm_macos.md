...
Date: 2026-05-05
Target example: `v7.2.4-ayastorm-r10`
...
export TARGET_TAG="v7.2.4-ayastorm-r10"
...
既存 worktree を使う場合も、ビルド前に `git status --short` で差分を確認します。不要な差分を含めたまま Release ビルドしないようにしてください。
...
```bash
cd "$WORK"
git clone https://github.com/FirestormViewer/3p-fmodstudio.git "$WORK/3p-fmodstudio"
```

ダウンロードした macOS 版 FMOD Studio API installer の `.dmg` を `$WORK/3p-fmodstudio` に置きます。

```bash
cd "$WORK/3p-fmodstudio"
ls -lh *.dmg
...
export FMOD_PACKAGE="$(find "$WORK/3p-fmodstudio" -maxdepth 1 -name 'fmodstudio-*-darwin64-*.tar.bz2' -print -quit)"
