...
source channel と prim 構成の不一致を log 出力で配置者が把握できるようにする。**source を正、prim 構成を従** で双方向に check:

```
[Stream3D] root abc-123 channel routing for 6ch Vorbis from http://192.168.0.2:8000/sample_5_1.ogg:
  source channel → prim
    FL  → 2 prim(s) ✓
    FR  → 2 prim(s) ✓
    C   → 0 prim   ← warn: "C content folded into BS.775 downmix on ch:L/R/M prims"
    LFE → 0 prim   ← warn: "LFE content folded into BS.775 downmix"
    SL  → 1 prim ✓
    SR  → 0 prim   ← warn: "SR content folded into BS.775 downmix"
  prim → source channel
    ch:FL × 2 ✓
    ch:M × 3 ✓ (BS.775 downmix)
```

#### 4.4.1 fallback 決定論

| ケース | fallback 先 | warn 文言 |
|---|---|---|
...
| ch:M prim, source 1ch | 直取り | warn 不要 |
...
- 同条件で連続評価 → key 同じ → log spam なし ✓

`prim_set_signature` は (root 配下の prim ID と ch 値の hash)。配置者が prim を追加 / 削除 / ch 変更すると key が変わって re-warn 発火。

#### 4.4.3 出力先 — 設定で制御

`Stream3DRoutingDiagnostic` (§6 設定) が `true` のとき LL_WARNS ログに出力、`false` (default) で完全静音。chat 通知には流さない (FormatUnsupported 等の hard failure とは独立、§6 参照)。
...
r10 で追加するユーザ設定は **1 件のみ**:

| 設定キー | 型 | 既定 | 効果 |
|---|---|---|---|
| `Stream3DRoutingDiagnostic` | bool | `false` | `true` で routing 診断 log を LL_WARNS で出力。`false` で完全静音。debug settings 配下 |

**設定方針**:

- 一般 listener は default `false` で log/chat とも完全静音、何も気付かず通常使用
- 配置者 (= AYA) は venue 構築 / テスト中だけ `true` にして自分の log を確認、本番は `false` に戻す
- chat 通知には流さない (FormatUnsupported / reconnect 失敗の chat 通知は本設定と独立、常時発火継続)
