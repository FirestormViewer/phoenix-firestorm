...
source channel と prim 構成の不一致を **chat 通知** で配置者が把握できるようにする。**source を正、prim 構成を従** で双方向に check し、各 fallback ケースごとに 1 行の chat 警告を出す。出力は `notifyStream3D` ヘルパ経由 (`"3D Stream: "` プレフィクスは helper が付与):

```
3D Stream: FL content folded into BS.775 downmix (source is 6ch, no ch:FL prim)
3D Stream: LFE content folded into BS.775 downmix (source is 6ch, no ch:LFE prim)
3D Stream: ch:FL prim playing L (source is 2ch)
3D Stream: ch:LFE prim silent (source is 2ch)
```
...
| ケース | fallback 先 | chat 通知文言 (notifyStream3D 引数、`"3D Stream: "` プレフィクスは helper 側で付与) |
|---|---|---|
| 6ch source, 専用 prim なし, ch:L/R/M prim あり | BS.775 downmix path に折り込む | `"FL content folded into BS.775 downmix (source is 6ch, no ch:FL prim)"` |
| 6ch source, 専用 prim なし, ch:L/R/M も無し | content drop | `"FL content has no destination — dropped (source is 6ch, no ch:L/R/M prim)"` |
| 2ch source, ch:FL/FR/C prim あり (compat 表で fallback 規定) | compat 表通り | `"ch:FL prim playing L (source is 2ch)"` (FR→R, C→M) |
| 1ch source, ch:FL/FR/C prim あり | M 再生 | `"ch:FL prim playing M (source is 1ch)"` |
| 1/2ch source, ch:LFE/SL/SR prim あり (silent 規定) | silent | `"ch:LFE prim silent (source is 2ch)"` (1ch のときは "1ch") |
| ch:L/R/M prim, source 1/2/6ch | matrix 通り (sum-to-mono / direct / BS.775) | 通知不要 (汎用 ch、常に妥当な fallback) |

r10.x 実装は `llpositionalstreammgr.cpp::emitRoutingDiagnostic` から英語固定文字列で chat に発火 (上表 emit 文言と完全一致)。多言語化 (XUI string table 経由) は将来課題。
...
`Stream3DRoutingDiagnostic` (§6 設定) が `true` のとき chat 通知 (`notifyStream3D` 経由) に出力、`false` (default) で完全静音。設定 UI は **Preferences (環境設定) > Sound タブ** の「3D Stream」slider 行直下に checkbox 「Show channel routing diagnostics in chat」として配置 (`panel_preferences_sound.xml` の `stream3d_routing_diagnostic_check`)。debug settings からも同キーで引けるが、運用 UI は Preferences を primary とする。
...
| Opus 6ch | ✓ 完了 (r9-opus 補完) | r9-opus で codec plugin を実装、実機 Icecast (`go-stream-live.com:8030/stream`) の 5.1 surround で r10 placement (per-channel `ch=FL/FR/C/LFE/SL/SR`) 動作を確認 |
| FLAC 6ch | △ コードレビューのみ | r9 §12.3 と同じ FMOD seek 制約 (Icecast / 単純 HTTP では開けない)。SMPTE→family 1 reorder は P3 reader 側で実装済、unit test 経路で確認 |
...
→ **詳細は `doc/spec_binaural_venue_reverb.md` (r11 仕様書) を参照**

要約:

- **Layer 2 (rendering) 初実装** — reverb (会場残響、9 venue) + lite-HRTF (ITD/ILD + air absorption)。§7.4 / §9 の hook point (`makeChannelForBinding()` の `set3DLevel(1.0f)`) を 0.0f に切替えて FMOD 既定 panner を無効化し、Layer 2 にバトンタッチする構造を r10 で確保済
- **配信者主導モデル** — `{venue:NAME}` / `{binaural:on|off}` / `{wetgain:N}` の 3 タグキーを root prim Desc に追加。listener 側 UI 改修ゼロ、debug settings 3 件 (sentinel 値方式) は実装/検証用のみ
- **SOFA per-source HRTF / Steam Audio 統合は r12+ に降格** — 当初 roadmap の SOFA 想定は投資対効果から見送り、r11 では非個人 SOFA を使わない lite-HRTF を採用
...
| 文言 | §4.4.1 の 5 ケースを英語固定文字列で `notifyStream3D` から chat 発火 (多言語化は将来課題) |
