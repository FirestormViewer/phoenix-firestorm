...
- スピーカー視聴の最適化 — ヘッドホン視聴を主、スピーカー視聴では DSP off に倒す逃げ道のみ用意
...
- G4. タグ書式に `{venue:NAME}` キーを追加、配置者が venue 種別を指定できる
- G5. 利用者側で venue を override する設定 (撮影用途) を提供
- G6. 設定で binaural / reverb を個別に on/off (スピーカー視聴 / dry 素材聴取の逃げ道)
- G7. 既存配置 (r8 / r10) が **タグ無改修で** binaural レンダリングの恩恵を受ける (lite-HRTF は default ON)
- G8. 3D Stream **専用** ChannelGroup を導入し、voice / UI / SFX / ambient world sound に reverb / binaural が漏れない
- G9. r10 の §5.3 受入条件 (dropout / CPU / URL 切替 / 互換マトリクス / 回帰) は **すべて維持**
- G10. r12+ で SOFA per-source HRTF を載せる場合の hook point を仕様書とコードに明記
...
r8 で確立した親プリム書式に **値の集合ではなく新キー** として追加する。`venue` は親プリム (= 音源宣言) にだけ意味がある。子プリム (placement) には書かない。

```
[3dstream-stereo:{url:http://example/sample_5_1.ogg}{venue:hall_medium}]
```
...
#### 4.1.0 旧 alias での venue 利用

r5 命名整理 (`project_ayastorm_r5_naming.md` / `docs/ayastorm-r5-naming-refactor.md`) で確立した恒久 alias `[ayastream-stereo:...]` でも venue キーは効く。
...
#### 4.1.1 設計判断: `venue` を子プリムに書かなかった理由

子プリムごとに venue を変える案 (= speaker 単位の reverb 種別) も検討したが棄却。

- 物理的に「同一会場内のスピーカーごとに残響種別が違う」状況は存在しない
- 6 prim placement で各 prim が異なる venue を宣言したらどう mix するかルール追加が必要
- 配信源 1 本につき venue 1 種、という対応が ライブ会場モデル (`spec_5_1ch_placement.md` §4.3) と整合

`venue` は **音源宣言の属性**であり、placement (子プリム) の属性ではない。
...
設定 `Stream3DBinauralRender = false` のときは `set3DLevel(1.0f)` のまま (= r10 と完全同一動作)、DSP は挿入しない。
...
- venue=dry または `Stream3DVenueReverb=false` のとき: DSP を bypass (= CPU ゼロ)
- それ以外: wet を `Stream3DVenueWetGain` 倍して dry に加算
...
### 4.5 listener-side venue override

利用者側の設定 `Stream3DVenueOverride` で配置者宣言を上書き可能 (撮影用途):

| 値 | 効果 |
|---|---|
| `tag` (default) | 配置者の `{venue:NAME}` に従う、未指定なら dry |
| `off` | 常に dry (reverb 無効化、=`Stream3DVenueReverb=false` 相当) |
| `<venue_name>` | venue 名を listener 側で固定 (例: `hall_medium`) |

これは `FSRenderHideOutsideParcel` の利用者側設定と同じ思想 (任意の区画で利用者判断で適用、機能本体の挙動とは独立)。memory `project_render_hide_outside_parcel_scope.md` 参照。
...
1. **binaural 単体検証 (lite-HRTF)**:
   - `voice_panning_test.ogg` を 1 prim mono 配置 (avatar 前方 5m)、`Stream3DBinauralRender=true` で再生
   - avatar を 360° 回転させ、音源方向が頭の動きに自然追従するか確認
   - 同じ素材で `Stream3DBinauralRender=false` (= r10 同等) と A/B 比較し、定位の体感差を確認
2. **air absorption 検証**:
   - `pink_noise_30s.ogg` を 1 prim 配置、avatar が prim から 5m / 25m / 50m に移動して HF が暗くなるか確認
3. **ITD 検証**:
   - `click_train_440Hz.ogg` を avatar 真横 (90°) に配置、左右どちらの耳から click が先に聞こえるか確認 (binaural ON / OFF で差が出るはず)
...
6. **listener override 検証**:
   - 配置者が `{venue:hall_medium}` を宣言した状態で、利用者が `Stream3DVenueOverride=cathedral` に設定、cathedral 残響が掛かるか
   - 利用者が `off` に設定で、配置者宣言があっても dry になるか
...
8. **回帰確認**:
   - `Stream3DBinauralRender=false` + `Stream3DVenueReverb=false` で r10 までの動作が完全一致 (dropout / CPU / placement / 互換マトリクス)
...
| listener override が tag/off/`<name>` の 3 形態で正しく動作 | ✓ PASS | (リリース時) |
| voice / UI sound / ambient world sound に venue reverb が漏れない | ✓ PASS | (リリース時) |
| binaural OFF + reverb OFF で r10 との挙動完全一致 (dropout 0、CPU 同等、placement 同じ) | ✓ PASS | (リリース時) |
...
| 設定キー | 型 | 既定 | 効果 | 設定 UI |
|---|---|---|---|---|
| `Stream3DBinauralRender` | bool | `true` | true で lite-HRTF DSP を挿入。false で r10 同等動作 (FMOD 既定 panner) | Preferences > Sound タブ |
| `Stream3DVenueReverb` | bool | `true` | true で venue convolution reverb DSP を有効化。false で全 venue 宣言を無視 (常に dry) | Preferences > Sound タブ |
| `Stream3DVenueOverride` | string | `tag` | 値は `tag` / `off` / venue 名。listener 側で配置者の venue 宣言を上書き | Preferences > Sound タブ |
| `Stream3DVenueWetGain` | F32 | `1.0` | 0.0〜2.0、venue reverb の wet 強度微調整。1.0 で IR のオリジナル強度 | debug settings (一般 UI には出さない) |

**設定方針**:

- 一般 listener は default で binaural / reverb 両方 ON、何も設定しなくても恩恵を受ける (G7)
- ヘッドホン以外で聴くユーザ / dry 素材を dry のまま聴きたいユーザは `Stream3DBinauralRender=false` で逃げ道
- 配置者は `{venue:NAME}` で venue 宣言、listener override 設定とは独立
- 設定切替は live (次の evaluateLinkset から効く、再起動不要)
- `Stream3DRoutingDiagnostic` (r10) は本書と独立、unchanged

memory `project_prefer_defaults_over_config.md` の方針 (= 多数の tuning キーより 1 つの妥当値) に従い、IR ごとの個別 wet / dry / pre-delay tuning は debug settings の `Stream3DVenueWetGain` 1 件に絞る。
...
- 新規キー `{venue:NAME}` を r8 / r9 / r10 viewer が受信した場合: 不正キーとして silent ignore (既存挙動)。r11 viewer のみが新規キーを解釈
- 同じ source URL に対して旧 viewer (r8/r9/r10) と新 viewer (r11) が共存可能
- r5 命名整理で確立した恒久 alias `[ayastream-stereo:...]` でも `{venue:NAME}` は同じく解釈される (§4.1.0)。新規 description 推奨は canonical `[3dstream-stereo:...]` 側だが、旧 alias 利用者の再編集は強制しない
...
- 設定追加 4 件 (§6)、削除 / リネームなし
- 新規 `LL_INFOS("Stream3DReverb")` channel で venue ロード結果 / IR ファイル不在を log
- 既存ログメッセージは無改修

### 7.4 FMOD API

- `Channel::set3DLevel(0.0f)` を r10 hook (`makeChannelForBinding()`) で反転 (binaural ON 時のみ)
- `Channel::addDSP(0, lite_hrtf_dsp)` を per-channel で挿入 (binaural ON 時のみ)
- `ChannelGroup::addDSP(end, venue_reverb_dsp)` を Stream3D group で挿入 (reverb ON 時のみ)
...
- **R5**. ChannelGroup 分離による副作用 ─ 既存 master volume / mute / 全体 setVolume が Stream3D に正しく伝播するか確認。継承関係が誤るとボリューム slider が効かない / mute が効かないなどの致命的 regression
...
- **R8**. `Stream3DBinauralRender=false` で r10 完全互換を保証する難しさ ─ ChannelGroup 分離は常に行うので、group の DSP が空 (DSP chain が空) でも素通しが完全 bit-identical かを §5.3 で確認
...
- r11 の binaural toggle (`Stream3DBinauralRender`) と独立な enum 設定 `Stream3DBinauralEngine = lite | sofa | off` への移行を検討
...
| ファイル | 役割 | r11 改修ポイント |
|---|---|---|
| `indra/llaudio/llpositionalstreammulti.cpp` | per-channel speaker bring-up | `makeChannelForBinding()` で `set3DLevel` の値を `Stream3DBinauralRender` に応じて切替、`addDSP(LiteHrtfDsp)` 挿入 |
...
| `indra/newview/llpositionalstreammgr.cpp` | description parser / binding mgr | `venue` キーの parse 追加、不正値 chat 通知、listener override 適用 |
| `indra/newview/llpositionalstreammgr.h` | 同 | `mVenue` フィールド追加、root prim 単位で保持 |
| `indra/newview/llvieweraudio.cpp` | listener pose 設定 | (改修なし、既存の `setListener()` 経路を DSP の per-frame param 取得元として利用) |
| `indra/newview/app_settings/settings.xml` | ユーザ設定 | `Stream3DBinauralRender` / `Stream3DVenueReverb` / `Stream3DVenueOverride` / `Stream3DVenueWetGain` 追加 |
| `indra/newview/skins/default/xui/en/panel_preferences_sound.xml` | Preferences UI | Sound タブに「Binaural rendering」「Venue reverb」セクション追加 |
...
| ID | 内容 | 主要成果物 | 受入リンク |
|---|---|---|---|
| **P0** | 仕様確定 + roadmap doc 同時更新 | 本書 (`spec_binaural_venue_reverb.md`) を AYA レビュー込みで確定、`docs/ayastorm-stream3d-roadmap.md` §3 r11 entry を「Steam Audio + 任意 SOFA」から「lite-HRTF + venue convolution reverb」へ書き換え。Steam Audio / 個人 SOFA は r12+ へ降格表記 | §1, §9.2 |
| **P1** | Stream3D ChannelGroup 分離 | `LLAudioEngine_FMODSTUDIO` に Stream3D group 追加、`makeChannelForBinding()` で `setChannelGroup` 呼出 | G8 |
| **P2** | LiteHrtfDsp 骨格 (DSP description, mono in → stereo out, 距離減衰だけ) | `lllitehrtfdsp.{h,cpp}` 新規 | G2, G9 |
...
| **P5** | `Stream3DBinauralRender` 設定追加 + hook 反転 | settings.xml 追加、`makeChannelForBinding()` で gate | G6, G9 |
...
| **P8** | venue タグ parser + listener override | `llpositionalstreammgr.cpp` の `venue` キー処理、`Stream3DVenueOverride` の 3 形態 | G4, G5 |
| **P9** | `Stream3DVenueReverb` / `Stream3DVenueWetGain` 設定 + venue reverb DSP の group 末尾挿入 | settings.xml 追加、reverb gate | G6 |
| **P10** | Preferences UI (Sound タブ) | `panel_preferences_sound.xml` に binaural / reverb セクション追加 | G6 |
| **P11** | 検証材料生成 + AYA Icecast hosting | `doc/r11/gen_test_material.sh` + 3 素材 | §5 |
| **P12** | §5.3 検証手順 1〜8 全件実行 | §5.5 受入表全行 PASS | §5.5 |
| **P13** | r8/r9/r10 回帰確認 | binaural OFF + reverb OFF で r10 完全一致、各リリースの §5.3 受入条件再実行 | G9 |
...
P0 (仕様 + roadmap doc 同時更新) は本書策定と同タイミングで roadmap §3 を整合させ、以降 r11 を進める誰もが同じ「lite-HRTF + venue reverb」前提で動けるようにする (= 旧 Steam Audio 案を root truth として残さない)。P0 完了後に P1 着手。P1 (group 分離) は両 DSP の前提なので最先着。P2-P5 (LiteHrtfDsp 系統) と P6-P9 (VenueReverbDsp 系統) は P1 完了後に独立並行可。P10 (Preferences UI) は両系統の設定追加 (P5 / P9) が出揃った後で配置。P12 (検証) は実装全完了後、P13 で回帰、P14 でクローズ。

### スコープ縮退オプション (リリース判断時)

- **縮退 A**: lite-HRTF の体感が薄く実機改善できない → `Stream3DBinauralRender` の default を `false` に倒して r10 同等を default にし、binaural は opt-in 機能として r11 出荷。lite-HRTF の改善は r11.x 以降
...
| カテゴリ | 工数 |
|---|---|
| P1 (Stream3D group 分離) | 0.5 日 |
| P2-P4 (LiteHrtfDsp 実装 + per-frame param) | 2.5 日 |
| P5 (binaural 設定 + hook 反転) | 0.5 日 |
| P6-P7 (VenueReverbDsp + bundled IR) | 3 日 |
| P8 (venue タグ + override) | 0.5 日 |
| P9 (reverb 設定 + group 挿入) | 0.5 日 |
| P10 (Preferences UI) | 0.5 日 |
| P11 (検証材料生成) | 0.5 日 |
| P12-P14 (検証 + 回帰 + クローズ) | 2 日 |
| **合計** | **9〜11 日** |

memory `project_release_workload_norms`: r9 / r10 が 3〜4 日、r11 (本案) は DSP 2 個新設 + ChannelGroup 改修で約 2.5 倍見込み。Steam Audio 案 (19-33 日) より大幅に軽い (= 3rd party SDK / 3 platform binary 配布の負債が無いため)。

リスク発火時の振れ幅:

- 最速 7 日 (lite-HRTF 体感 OK + 縮退発生せず + IR ライセンスがすんなり通る)
- 平均 9〜11 日 (上記カテゴリ通り)
- 最悪 14 日 (ChannelGroup 分離の master volume 問題で縮退 C / IR 選定再やり直し / lite-HRTF re-tuning)
