...
public:
    ...
    std::optional<std::string> serializeUser(std::string& error) const;
    bool saveUserFile(const std::filesystem::path& path,std::string& error) const;
    bool loadUserFile(const std::filesystem::path& path,std::string& error);
private:
    std::map<std::string,std::shared_ptr<LLVKColor::Slot>> mLoaded;
    std::map<std::string,std::shared_ptr<LLVKColor::Slot>> mUser;
    std::map<std::string,std::shared_ptr<LLVKColor::Slot>> mRuntime;
};
