...
// <VulkanStorm> Win32 Vulkan surface creation. Resolved at runtime so llwindow
// stays free of Vulkan headers/link deps; the typed handle is interpreted by
// the llvulkan backend which passes its VkInstance here.
bool LLWindowWin32::createVulkanSurface(void* vk_instance, void* out_surface)
{
    if (!vk_instance || !out_surface || !mWindowHandle)
    {
        return false;
    }

    // vkCreateWin32SurfaceKHR is an instance-extension function; fetch it via
    // vkGetInstanceProcAddr from the already-loaded loader (vulkan-1.dll).
    HMODULE vulkan_loader = GetModuleHandleA("vulkan-1.dll");
    if (!vulkan_loader)
    {
        vulkan_loader = LoadLibraryA("vulkan-1.dll");
    }
    if (!vulkan_loader)
    {
        LL_WARNS("Window") << "Vulkan surface: vulkan-1.dll not available" << LL_ENDL;
        return false;
    }

    typedef void* (WINAPI* PFN_vkGetInstanceProcAddr_t)(void*, const char*);
    PFN_vkGetInstanceProcAddr_t get_proc =
        (PFN_vkGetInstanceProcAddr_t)GetProcAddress(vulkan_loader, "vkGetInstanceProcAddr");
    if (!get_proc)
    {
        LL_WARNS("Window") << "Vulkan surface: vkGetInstanceProcAddr not found" << LL_ENDL;
        return false;
    }

    // VkWin32SurfaceCreateInfoKHR layout (matches the Vulkan spec; we avoid the
    // headers here to keep llwindow GL-only). Fields in declaration order.
    struct VkWin32SurfaceCreateInfoKHR_t
    {
        unsigned int sType;          // VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR = 1000009000
        const void*  pNext;
        unsigned int flags;
        HINSTANCE    hinstance;
        HWND         hwnd;
    ...
    typedef int (WINAPI* PFN_vkCreateWin32SurfaceKHR_t)(void*, const VkWin32SurfaceCreateInfoKHR_t*, const void*, void**);
    PFN_vkCreateWin32SurfaceKHR_t create_surface =
        (PFN_vkCreateWin32SurfaceKHR_t)get_proc(vk_instance, "vkCreateWin32SurfaceKHR");
    if (!create_surface)
    {
        LL_WARNS("Window") << "Vulkan surface: vkCreateWin32SurfaceKHR not exposed by ICD" << LL_ENDL;
        return false;
    }

    VkWin32SurfaceCreateInfoKHR_t create_info;
    create_info.sType = 1000009000; // VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR
    create_info.pNext = NULL;
    create_info.flags = 0;
    create_info.hinstance = mhInstance;
    create_info.hwnd = mWindowHandle;

    int result = create_surface(vk_instance, &create_info, NULL, (void**)out_surface);
    if (result != 0) // VK_SUCCESS == 0
    {
        LL_WARNS("Window") << "Vulkan surface: vkCreateWin32SurfaceKHR failed (VkResult " << result << ")" << LL_ENDL;
        return false;
    }

    LL_INFOS("Window") << "Created Vulkan Win32 surface for window" << LL_ENDL;
    return true;
}
// </VulkanStorm>
