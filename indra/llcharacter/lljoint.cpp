...
#include "llmutex.h" // <FS:minerjr> Add share_mutex to fix JointKey::construct lockup
//<FS:ND> Query by JointKey rather than just a string, the key can be a U32 index for faster lookup
#include <unordered_map>

std::unordered_map<std::string, U32> mpStringToKeys;
std::shared_mutex mpStringToKeysMutex; // <FS:minerjr> Add share_mutex to fix JointKey::construct lockup

JointKey JointKey::construct(const std::string& aName)
{
    {// <FS:minerjr> Add share_mutex to fix JointKey::construct lockup
    std::shared_lock lock(mpStringToKeysMutex); // </FS:minerjr> Add a shared lock for reading the mpStringToKeys unordered_map.
    ...
    {
    std::unique_lock lock(mpStringToKeysMutex);// </FS:minerjr> Add a unique lock for writing to the mpStringToKeys unordered_map.
