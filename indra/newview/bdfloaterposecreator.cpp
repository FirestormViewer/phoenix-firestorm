...
BDFloaterPoseCreator::BDFloaterPoseCreator(const LLSD& key) : LLFloater(key), mJointScrolls()
{
...
void BDFloaterPoseCreator::onPoseStartStop()
{
    ...
    if(gDragonAnimator.mPoseCreatorMotion)
    {
        ...
        if (!tmotion)
        {
            std::string full_path = gDirUtilp->getExpandedFilename(LL_PATH_USER_SETTINGS, "animations", "_poser_temp.anim");
...
bool BDFloaterPoseCreator::onPoseExport()
{
    ...
    std::string full_path = gDirUtilp->getExpandedFilename(LL_PATH_USER_SETTINGS, "animations", motion_name + ".anim");
...
{
    std::string outfilename = gDirUtilp->getExpandedFilename(LL_PATH_USER_SETTINGS, "animations", filename);
    S32 file_size;
    BOOL success = FALSE;
    LLAPRFile infile;
