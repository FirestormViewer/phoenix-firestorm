            ...
            if (info.mIsCategory)
            {
                ...
                else if (gInventory.isCategoryComplete(info.mUUID))
                {
                    ...
                    for (LLInventoryModel::cat_array_t::const_iterator it = categories->begin();
                         ...
                         ++it)
                    {
                        mFetchFolderQueue.emplace_back((*it)->getUUID(), info.mFetchType);
                    }
                ...
                else if (mFetchTimer.getElapsedTimeF32() > mMaxTimeBetweenFetches)
                {
                    ...
                    if (mNumFetchRetries++ < MAX_FETCH_RETRIES)
                    {
                        ...
                        mFetchFolderQueue.emplace_back(info);
                    }
                    mTimelyFetchPending = false;
                    mFetchTimer.reset();
                    break;
        ...
        else
        {
            ...
            if (categories)
            {
                for (LLInventoryModel::cat_array_t::const_iterator it = categories->begin();
                     ...
                     ++it)
                {
                    mFetchFolderQueue.emplace_back(FetchQueueInfo((*it)->getUUID(), FT_RECURSIVE));
                }
            }
        }
        ...
        if (fetch_info.mIsCategory)
        {
            ...
            else
            {
                ...
                if (cat)
                {
                    ...
                    // <FS/> [FIRE-34403] Removing the else here allows the inventory to do a full fetch
                    //else
                    //{
