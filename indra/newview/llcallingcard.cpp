...
{
	for( std::map<LLUUID, std::set<LLFriendObserver*> >::iterator itr = aContainer.begin(); itr != aContainer.end(); ++itr )
	{
		for( std::set<LLFriendObserver*>::iterator itr2 = itr->second.begin(); itr2 != itr->second.end(); ++itr2 )
		{
			if( *itr2 == aObserver )
			{
