...
// FSFloaterIMContainer
//
FSFloaterIMContainer::FSFloaterIMContainer(const LLSD& seed, const Params& params /*= getDefaultParams()*/)
:	LLMultiFloater(seed, params),
	...
	mInitialized(false)
{
    mEnableCallbackRegistrar.add("IMFloaterContainer.Check", boost::bind(&FSFloaterIMContainer::isActionChecked, this, _2));
	mCommitCallbackRegistrar.add("IMFloaterContainer.Action", boost::bind(&FSFloaterIMContainer::onCustomAction,  this, _2));
	
	mEnableCallbackRegistrar.add("Avatar.CheckItem",  boost::bind(&FSFloaterIMContainer::checkContextMenuItem,	this, _2));
	mEnableCallbackRegistrar.add("Avatar.EnableItem", boost::bind(&FSFloaterIMContainer::enableContextMenuItem,	this, _2));
	mEnableCallbackRegistrar.add("Avatar.VisibleItem", boost::bind(&FSFloaterIMContainer::visibleContextMenuItem,	this, _2));
    mCommitCallbackRegistrar.add("Avatar.DoToSelected", boost::bind(&FSFloaterIMContainer::doToSelected, this, _2));
    
    mCommitCallbackRegistrar.add("Group.DoToSelected", boost::bind(&FSFloaterIMContainer::doToSelectedGroup, this, _2));
...
void FSFloaterIMContainer::sessionAdded(const LLUUID& session_id, const std::string& name, const LLUUID& other_participant_id, BOOL has_offline_msg)
{
	...
	FSFloaterIMSessionTab::addToHost(session_id);
}

void FSFloaterIMContainer::sessionActivated(const LLUUID& session_id, const std::string& name, const LLUUID& other_participant_id)
{
	selectConversationPair(session_id, true);
	collapseMessagesPane(false);
}

void FSFloaterIMContainer::sessionVoiceOrIMStarted(const LLUUID& session_id)
{
	addConversationListItem(session_id);
...
// static
void FSFloaterIMContainer::onCurrentChannelChanged(const LLUUID& session_id)
{
...
BOOL FSFloaterIMContainer::postBuild()
{
    ...
    mStubTextBox->setURLClickedCallback(boost::bind(&FSFloaterIMContainer::returnFloaterToHost, this));
	...
	mConversationsListPanel->setDoubleClickCallback(boost::bind(&FSFloaterIMContainer::doToSelected, this, LLSD("im")));

	// The resize limits for FSFloaterIMContainer should be updated, based on current values of width of conversation and message panels
	mConversationsPane->getResizeBar()->setResizeListener(boost::bind(&FSFloaterIMContainer::assignResizeLimits, this));
	...
	mConversationsEventStream.listen("ConversationsRefresh", boost::bind(&FSFloaterIMContainer::onConversationModelEvent, this, _1));
	...
	mExpandCollapseBtn->setClickedCallback(boost::bind(&FSFloaterIMContainer::onExpandCollapseButtonClicked, this));
	mStubCollapseBtn = getChild<LLButton>("stub_collapse_btn");
	mStubCollapseBtn->setClickedCallback(boost::bind(&FSFloaterIMContainer::onStubCollapseButtonClicked, this));
	getChild<LLButton>("speak_btn")->setClickedCallback(boost::bind(&FSFloaterIMContainer::onSpeakButtonClicked, this));

	childSetAction("add_btn", boost::bind(&FSFloaterIMContainer::onAddButtonClicked, this));
	...
	mMicroChangedSignal = LLVoiceClient::getInstance()->MicroChangedCallback(boost::bind(&FSFloaterIMContainer::updateSpeakBtnState, this));
	...
	LLAvatarNameCache::addUseDisplayNamesCallback(boost::bind(&FSFloaterIMContainer::processParticipantsStyleUpdate, this));
...
void FSFloaterIMContainer::onStubCollapseButtonClicked()
{
	collapseMessagesPane(true);
}

void FSFloaterIMContainer::onSpeakButtonClicked()
{
	LLAgent::toggleMicrophone("speak");
	updateSpeakBtnState();
}
void FSFloaterIMContainer::onExpandCollapseButtonClicked()
{
...
FSFloaterIMContainer* FSFloaterIMContainer::findInstance()
{
	return LLFloaterReg::findTypedInstance<FSFloaterIMContainer>("im_container");
}

FSFloaterIMContainer* FSFloaterIMContainer::getInstance()
{
	return LLFloaterReg::getTypedInstance<FSFloaterIMContainer>("im_container");
}

// Update all participants in the conversation lists
void FSFloaterIMContainer::processParticipantsStyleUpdate()
{
...
// static
void FSFloaterIMContainer::idle(void* user_data)
{
	FSFloaterIMContainer* self = static_cast<FSFloaterIMContainer*>(user_data);
...
bool FSFloaterIMContainer::onConversationModelEvent(const LLSD& event)
{
	// For debug only
	//std::ostringstream llsd_value;
	//llsd_value << LLSDOStreamer<LLSDNotationFormatter>(event) << std::endl;
	//llinfos << "FSFloaterIMContainer::onConversationModelEvent, event = " << llsd_value.str() << llendl;
	// end debug
...
void FSFloaterIMContainer::tabClose()
{
	if (mTabContainer->getTabCount() == 0)
	{
		// Do not close the container when every conversation is torn off because the user
...
void FSFloaterIMContainer::showStub(bool stub_is_visible)
{
    S32 tabCount = 0;
...
void FSFloaterIMContainer::returnFloaterToHost()
{
...
void FSFloaterIMContainer::setVisible(BOOL visible)
...
void FSFloaterIMContainer::setVisibleAndFrontmost(BOOL take_focus, const LLSD& key)
{
	LLMultiFloater::setVisibleAndFrontmost(take_focus, key);
    selectConversationPair(getSelectedSession(), false, take_focus);
}

void FSFloaterIMContainer::updateResizeLimits()
{
	LLMultiFloater::updateResizeLimits();
	assignResizeLimits();
}

void FSFloaterIMContainer::collapseMessagesPane(bool collapse)
{
...
void FSFloaterIMContainer::collapseConversationsPane(bool collapse, bool save_is_allowed /*=true*/)
{
...
void FSFloaterIMContainer::reshapeFloaterAndSetResizeLimits(bool collapse, S32 delta_width)
{
...
void FSFloaterIMContainer::assignResizeLimits()
{
...
void FSFloaterIMContainer::onAddButtonClicked()
{
    LLView * button = findChild<LLView>("conversations_pane_buttons_expanded")->findChild<LLButton>("add_btn");
    LLFloater* root_floater = gFloaterView->getParentFloater(this);
    LLFloaterAvatarPicker* picker = LLFloaterAvatarPicker::show(boost::bind(&FSFloaterIMContainer::onAvatarPicked, this, _1), TRUE, TRUE, TRUE, root_floater->getName(), button);
...
void FSFloaterIMContainer::onAvatarPicked(const uuid_vec_t& ids)
{
    if (ids.size() == 1)
    {
...
void FSFloaterIMContainer::onCustomAction(const LLSD& userdata)
{
...
BOOL FSFloaterIMContainer::isActionChecked(const LLSD& userdata)
{
...
void FSFloaterIMContainer::setSortOrderSessions(const LLConversationFilter::ESortOrderType order)
{
...
void FSFloaterIMContainer::setSortOrderParticipants(const LLConversationFilter::ESortOrderType order)
{
	LLConversationSort old_order = mConversationViewModel.getSorter();
...
void FSFloaterIMContainer::setSortOrder(const LLConversationSort& order)
{
...
void FSFloaterIMContainer::getSelectedUUIDs(uuid_vec_t& selected_uuids)
{
...
const LLConversationItem * FSFloaterIMContainer::getCurSelectedViewModelItem()
{
...
void FSFloaterIMContainer::getParticipantUUIDs(uuid_vec_t& selected_uuids)
{
...
void FSFloaterIMContainer::doToParticipants(const std::string& command, uuid_vec_t& selectedIDS)
{
...
void FSFloaterIMContainer::doToSelectedConversation(const std::string& command, uuid_vec_t& selectedIDS)
{
    //Find the conversation floater associated with the selected id
...
void FSFloaterIMContainer::doToSelected(const LLSD& userdata)
{
    std::string command = userdata.asString();
    const LLConversationItem * conversationItem = getCurSelectedViewModelItem();
...
void FSFloaterIMContainer::doToSelectedGroup(const LLSD& userdata)
{
...
bool FSFloaterIMContainer::enableContextMenuItem(const LLSD& userdata)
{
...
bool FSFloaterIMContainer::enableContextMenuItem(const std::string& item, uuid_vec_t& uuids)
{
...
bool FSFloaterIMContainer::checkContextMenuItem(const LLSD& userdata)
{
...
bool FSFloaterIMContainer::checkContextMenuItem(const std::string& item, uuid_vec_t& uuids)
{
    if (uuids.size() == 1)
    {
		if ("is_blocked" == item)
...
bool FSFloaterIMContainer::visibleContextMenuItem(const LLSD& userdata)
{
	const std::string& item = userdata.asString();
...
void FSFloaterIMContainer::showConversation(const LLUUID& session_id)
{
...
void FSFloaterIMContainer::clearAllFlashStates()
{
	conversations_widgets_map::iterator widget_it = mConversationsWidgets.begin();
	for (;widget_it != mConversationsWidgets.end(); ++widget_it)
	{
...
void FSFloaterIMContainer::selectConversation(const LLUUID& session_id)
{
    selectConversationPair(session_id, true);
}
...
void FSFloaterIMContainer::selectNextConversationByID(const LLUUID& uuid)
{
...
BOOL FSFloaterIMContainer::selectConversationPair(const LLUUID& session_id, bool select_widget, bool focus_floater/*=true*/)
{
    BOOL handled = TRUE;
    LLFloaterIMSessionTab* session_floater = LLFloaterIMSessionTab::findConversation(session_id);
...
void FSFloaterIMContainer::setTimeNow(const LLUUID& session_id, const LLUUID& participant_id)
{
...
void FSFloaterIMContainer::setNearbyDistances()
{
...
LLConversationItem* FSFloaterIMContainer::addConversationListItem(const LLUUID& uuid, bool isWidgetSelected /*= false*/)
{
...
bool FSFloaterIMContainer::removeConversationListItem(const LLUUID& uuid, bool change_focus)
{
...
LLConversationViewSession* FSFloaterIMContainer::createConversationItemWidget(LLConversationItem* item)
{
...
LLConversationViewParticipant* FSFloaterIMContainer::createConversationViewParticipant(LLConversationItem* item)
{
...
bool FSFloaterIMContainer::enableModerateContextMenuItem(const std::string& userdata)
{
...
bool FSFloaterIMContainer::isGroupModerator()
{
...
void FSFloaterIMContainer::moderateVoice(const std::string& command, const LLUUID& userID)
{
...
bool FSFloaterIMContainer::isMuted(const LLUUID& avatar_id)
{
	const LLSpeaker * speakerp = getSpeakerOfSelectedParticipant(getSpeakerMgrForSelectedParticipant());
	return NULL == speakerp ? true : speakerp->mStatus == LLSpeaker::STATUS_MUTED;
}

void FSFloaterIMContainer::moderateVoiceAllParticipants(bool unmute)
{
...
// static
void FSFloaterIMContainer::confirmMuteAllCallback(const LLSD& notification, const LLSD& response)
{
...
void FSFloaterIMContainer::moderateVoiceParticipant(const LLUUID& avatar_id, bool unmute)
{
	LLIMSpeakerMgr * speaker_managerp = dynamic_cast<LLIMSpeakerMgr *>(getSpeakerMgrForSelectedParticipant());
...
LLSpeakerMgr * FSFloaterIMContainer::getSpeakerMgrForSelectedParticipant()
{
...
LLSpeaker * FSFloaterIMContainer::getSpeakerOfSelectedParticipant(LLSpeakerMgr * speaker_managerp)
{
	if (NULL == speaker_managerp)
	{
		llwarns << "Speaker manager is missing" << llendl;
...
void FSFloaterIMContainer::toggleAllowTextChat(const LLUUID& participant_uuid)
{
	LLIMSpeakerMgr * speaker_managerp = dynamic_cast<LLIMSpeakerMgr*>(getSpeakerMgrForSelectedParticipant());
	if (NULL != speaker_managerp)
	{
...
void FSFloaterIMContainer::toggleMute(const LLUUID& participant_id, U32 flags)
{
...
void FSFloaterIMContainer::openNearbyChat()
{
...
void FSFloaterIMContainer::onNearbyChatClosed()
{
...
void FSFloaterIMContainer::reSelectConversation()
{
...
void FSFloaterIMContainer::updateSpeakBtnState()
{
...
bool FSFloaterIMContainer::isConversationLoggingAllowed()
{
	return gSavedPerAccountSettings.getS32("KeepConversationLogTranscripts") > 0;
}

void FSFloaterIMContainer::flashConversationItemWidget(const LLUUID& session_id, bool is_flashes)
{
...
bool FSFloaterIMContainer::isScrolledOutOfSight(LLConversationViewSession* conversation_item_widget)
{
...
BOOL FSFloaterIMContainer::handleKeyHere(KEY key, MASK mask )
{
	if(mask == MASK_ALT)
	{
		if (KEY_RETURN == key )
...
bool FSFloaterIMContainer::selectAdjacentConversation(bool focus_selected)
{
...
bool FSFloaterIMContainer::selectNextorPreviousConversation(bool select_next, bool focus_selected)
{
	if (mConversationsWidgets.size() > 1)
	{
		LLFolderViewItem* new_selection = NULL;
...
void FSFloaterIMContainer::expandConversation()
{
...
void FSFloaterIMContainer::closeFloater(bool app_quitting/* = false*/)
{
