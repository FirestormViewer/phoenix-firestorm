...
bool LLPanelEventInfo::processEventInfoReply(LLEventInfo event)
{
    ...
    if (LLSimInfo* sim_info = LLWorldMap::getInstance()->simInfoFromPosGlobal(event.mPosGlobal))
    {
        mTBLocation->setText(LLSLURL(event.mSimName, sim_info->getGlobalOrigin(), event.mPosGlobal).getSLURLString());
    }
    else
    {
        mTBLocation->setText(LLTrans::getString("LoadingData"));
