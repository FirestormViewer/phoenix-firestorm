...
static S32 MORPH_MESHES[] = { LLAvatarAppearanceDefines::MESH_ID_HEAD,
	LLAvatarAppearanceDefines::MESH_ID_UPPER_BODY,
	LLAvatarAppearanceDefines::MESH_ID_LOWER_BODY };



// attempt to find the cached deform table.  if not found, generate request for it to be built and
// send to deform worker
LLDeformedVolume::LLDeformTable* LLDeformedVolume::getDeformTable(LLVolume* source, LLVOAvatar* avatar2,
																  const LLMeshSkinInfo* skin, S32 face,
																  LLPointer<LLDrawable> drawable)
{
	// see if we can find this deform table in the cache
	LLDeformTableCacheIndex cache_index(source->getParams().getSculptID(), face,
	...
	// send a request to the deformer worker.
	...
	// we copy this data now so that when we do the work in
	// the second thread, there's no chance it will be clobbered
	
	//
	// fill volume vertex positions/normals
	//
	const LLVolumeFace& source_face = source->getVolumeFace(face);
	request->mVolumePositions.resize(source_face.mNumVertices);
	request->mVolumeNormals.resize(source_face.mNumVertices);
	...
	for (S32 i = 0; i < source_face.mNumVertices; i++)
	{
		...
		LLVector3 normal(source_face.mNormals[i].getF32ptr());
		normal.normalize();
		request->mVolumeNormals[i] = normal;
	}
	
	// scale and matrix constants
	// maps volume space to object (avatar) space
	LLMatrix4a bind_shape_matrix_a;
	request->mBindShapeMatrix = skin->mBindShapeMatrix;
	...
	//
	// compute the base avatar shape for this deformed volume
	//
	
	// create default avatar
	LLPointer<LLVOAvatar> avatar = (LLVOAvatar*)gObjectList.createObjectViewer(LL_PCODE_LEGACY_AVATAR, gAgent.getRegion());
	avatar->mIsDummy = TRUE;
	avatar->mSpecialRenderMode = 2;
	
	avatar->updateJointLODs();
	
	// apply base visual_params
	LLSD visual_params = skin->mDeform["visual_params"];
	
	// at this point i planned to loop over the map and set the parameters it
	// specified.  but i learned that LLSD does not implement LLSD::mapBegin() and
	// LLSD::mapEnd().  (they are defined, just not implemented.  yay.)  so instead
	// i loop over all the possible visual params in the avatar, and see if they are
	// present in the map.
	LLVisualParam* this_param = avatar->getFirstVisualParam();
	
	while(this_param != NULL)
	{
		std::string name = this_param->getName();
		if (visual_params.has(name))
		{
			avatar->setVisualParamWeight(name.c_str(), visual_params[name].asReal());
		}
		
		this_param = avatar->getNextVisualParam();
	}
	
	// and apply this parameters
	avatar->updateVisualParams();
	
	
	// extract the data from this avatar, store it in the request
	S32 num_meshes = sizeof(MORPH_MESHES)/sizeof(S32);
	request->mAvatarIndices.resize(num_meshes);
	request->mAvatarPositions.resize(num_meshes);
	request->mAvatarNormals.resize(num_meshes);
	...
	for (S32 j = 0; j < num_meshes; j++)
	{
		...
		request->mAvatarIndices[j].resize(num_triangles * 3);
		for (S32 k = 0; k < num_triangles; k++)
		{
			...
			request->mAvatarIndices[j][k * 3 + 0] = index0;
			request->mAvatarIndices[j][k * 3 + 1] = index1;
			request->mAvatarIndices[j][k * 3 + 2] = index2;
		}
		
		// store avatar vertices/normals
		const LLVector4a* base_positions = avatar_mesh->getCoords();
		const LLVector4a* base_normals = avatar_mesh->getNormals();
		S32 num_vertices = avatar_mesh->getNumVertices();
		request->mAvatarPositions[j].resize(num_vertices);
		request->mAvatarNormals[j].resize(num_vertices);
		for (S32 k = 0; k < num_vertices; k++)
		{
			request->mAvatarPositions[j][k] = LLVector3(base_positions[k].getF32ptr());
			request->mAvatarNormals[j][k] = LLVector3(base_normals[k].getF32ptr());
			request->mAvatarNormals[j][k].normalize();
		}
...
void LLDeformedVolume::computeDeformTable(LLPointer<LLDeformerWorker::Request> request)
{
	...
	{
		// this table will very infrequently fill - so make space in simple way:
	...
	// build mapping
	deform_table.mMap.resize(request->mVertexCount);
	
	
	// find matrices (vert and normal)
	LLMatrix4a vertex_matrix;
	vertex_matrix.loadu(request->mBindShapeMatrix);
	
	LLMatrix4 normal_matrix_noA = request->mBindShapeMatrix;
	normal_matrix_noA.invert_real(); // normals are transformed by inverse transpose
	normal_matrix_noA.transpose();
	...
	for (S32 i = 0; i < request->mVertexCount; i++)
	{
		...
		vertex_matrix.affineTransform(source_position, source_position_transformed);
		...
		{
			deform_table.mMap[i].mMesh[k] = (U8)closest_mesh;
			deform_table.mMap[i].mVertex[k] = closest_vertex[k];
			deform_table.mMap[i].mWeight[k] = closest_vertex_weights[k];
		}
	}
...
BOOL LLDeformedVolume::deform(LLVolume* source, LLVOAvatar* avatar,
							  const LLMeshSkinInfo* skin, S32 face, LLPointer<LLDrawable> drawable)
{
	...
	LLDeformTable* deform_table = getDeformTable(source, avatar, skin, face, drawable);
	...
	for (S32 i = 0; i < source_face.mNumVertices; i++)
	{
		...
		{
			U16 closest_vertex = deform_table->mMap[i].mVertex[j];
			S32 closest_mesh = deform_table->mMap[i].mMesh[j];
			F32 closest_weight = deform_table->mMap[i].mWeight[j];
			
			// find the current mesh
			LLPolyMesh* avatar_mesh = avatar->getMesh(MORPH_MESHES[closest_mesh]);
			// and get positions for and morphed vertices
			const LLVector4a* morph_positions = avatar_mesh->getCoords();
			
			// find the movement of this vertex due to morphing
			LLVector4a base_position;
			base_position.load3(deform_table->mBasePositions[closest_mesh][closest_vertex].mV);
			LLVector4a morph_delta;
			morph_delta.setSub(morph_positions[closest_vertex], base_position);
