...
// ## Zi: Pie menu
	...
	LLPieMenu::Params pieParams;
	pieParams.label(LLTrans::getString("BodyPartsRightArm"));
	pieParams.name(pieParams.label);
	pieParams.visible(false);
	gPieAttachBodyPartMenus[1] = LLUICtrlFactory::create<LLPieMenu> (pieParams);

	pieParams.label(LLTrans::getString("BodyPartsHead"));
	pieParams.name(pieParams.label);
	gPieAttachBodyPartMenus[2] = LLUICtrlFactory::create<LLPieMenu> (pieParams);

	pieParams.label(LLTrans::getString("BodyPartsLeftArm"));
	pieParams.name(pieParams.label);
	gPieAttachBodyPartMenus[3] = LLUICtrlFactory::create<LLPieMenu> (pieParams);

	gPieAttachBodyPartMenus[4] = NULL;

	pieParams.label(LLTrans::getString("BodyPartsLeftLeg"));
	pieParams.name(pieParams.label);
	gPieAttachBodyPartMenus[5] = LLUICtrlFactory::create<LLPieMenu> (pieParams);

	pieParams.label(LLTrans::getString("BodyPartsTorso"));
	pieParams.name(pieParams.label);
	gPieAttachBodyPartMenus[6] = LLUICtrlFactory::create<LLPieMenu> (pieParams);

	pieParams.label(LLTrans::getString("BodyPartsRightLeg"));
	pieParams.name(pieParams.label);
	gPieAttachBodyPartMenus[7] = LLUICtrlFactory::create<LLPieMenu> (pieParams);

	gPieDetachBodyPartMenus[0] = NULL;

	pieParams.label(LLTrans::getString("BodyPartsRightArm"));
	pieParams.name(pieParams.label);
	gPieDetachBodyPartMenus[1] = LLUICtrlFactory::create<LLPieMenu> (pieParams);

	pieParams.label(LLTrans::getString("BodyPartsHead"));
	pieParams.name(pieParams.label);
	gPieDetachBodyPartMenus[2] = LLUICtrlFactory::create<LLPieMenu> (pieParams);

	pieParams.label(LLTrans::getString("BodyPartsLeftArm"));
	pieParams.name(pieParams.label);
	gPieDetachBodyPartMenus[3] = LLUICtrlFactory::create<LLPieMenu> (pieParams);

	gPieDetachBodyPartMenus[4] = NULL;

	pieParams.label(LLTrans::getString("BodyPartsLeftLeg"));
	pieParams.name(pieParams.label);
	gPieDetachBodyPartMenus[5] = LLUICtrlFactory::create<LLPieMenu> (pieParams);

	pieParams.label(LLTrans::getString("BodyPartsTorso"));
	pieParams.name(pieParams.label);
	gPieDetachBodyPartMenus[6] = LLUICtrlFactory::create<LLPieMenu> (pieParams);

	pieParams.label(LLTrans::getString("BodyPartsRightLeg"));
	pieParams.name(pieParams.label);
	gPieDetachBodyPartMenus[7] = LLUICtrlFactory::create<LLPieMenu> (pieParams);
// ## Zi: Pie menu
		...
		else
		{
				 ...
				 ++iter)
			{
				...
				if (attachment->getGroup() == i)
				{
					...
					LLPieSlice::Params slice_params;	// ## Zi: Pie menu
					...
					LLPieSlice* slice = LLUICtrlFactory::create<LLPieSlice>(slice_params);
		 ...
		 ++iter)
	{
		...
		if (attachment->getGroup() == 8)
		{
			...
			slice_params.on_enable.parameter = iter->first;
			slice = LLUICtrlFactory::create<LLPieSlice>(slice_params);
	...
	for (S32 group = 0; group < 8; group++)
	{
			 ...
			 attach_it != attachment_pie_menu_map.end(); ++attach_it)
		{
			...
			if (attachment)
			{
				...
				// ## Zi: Pie menu
				LLPieSlice::Params slice_params;
