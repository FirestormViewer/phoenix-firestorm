...
function install_desktop_entry()
{
    ...
    local desktop_entry="\
[Desktop Entry]\n\
Name=AYAstorm Viewer\n\
Comment=Client for accessing 3D virtual worlds\n\
Exec=${installation_prefix}/ayastorm\n\
...
StartupWMClass=do-not-directly-run-ayastorm-bin"

    echo " - Installing menu entries in ${desktop_entries_dir}"
    mkdir -vp "${desktop_entries_dir}"
    echo -e $desktop_entry > "${desktop_entries_dir}/ayastorm-viewer.desktop" || "Failed to install application menu!"
}
