...
BDFloaterPoser::BDFloaterPoser(const LLSD& key) : LLFloater(key)
{
...
bool BDFloaterPoser::onPoseSave()
{
    ...
    if (!file.is_open())
    {
        LL_WARNS("GroupMute") << "Unable to pose!" << LL_ENDL;
        return false;
    }
...
bool BDFloaterPoser::havePermissionToAnimateAvatar(LLVOAvatar *avatar)
{
    if (!avatar || avatar->isDead())
        return false;
...
void BDFloaterPoser::loadPoseRotations(std::string name, LLVector3 *rotations)
{
    ...
    if (!name.empty())
    {
        filename = gDirUtilp->getExpandedFilename(LL_PATH_USER_SETTINGS, "poses", filename + ".xml");
    }
...
void BDFloaterPoser::onAvatarsRefresh()
{
    ...
    for (LLCharacter* character : LLCharacter::sInstances)
    {
        ...
        if (avatar && !avatar->isControlAvatar() && avatar->isSelf())
        {
